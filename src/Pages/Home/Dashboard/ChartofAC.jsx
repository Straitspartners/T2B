...
function ChartofAccounts() {
  ...
  const fetchAllDashboardData = async () => {
    try {
      ...
      // Chart of Accounts API endpoint
      const endpoint = `https://tallytobooks-backend-bnezgff5eehsftfj.centralindia-01.azurewebsites.net/api/coadashboard/`;
      ...
      console.log("COA dashboard response status:", response.status);
      console.log("COA dashboard response ok:", response.ok);

      if (!response.ok) {
        const errorText = await response.text();
        console.error("COA dashboard response error:", errorText);
        throw new Error(`Failed to fetch COA dashboard data: ${response.status} - ${errorText}`);
      }

      const data = await response.json();
      console.log("COA dashboard raw API response:", data);
      ...
      // Transform recent activities data from all_ledgers for Chart of Accounts
      const activitiesData = data.all_ledgers || [];
      const transformedActivities = activitiesData.map((account, index) => ({
        sNo: index + 1,
        account_name: account.account_name || account.name || 'N/A',
        account_code: account.account_code || account.ledger_code || account.code || 'N/A',
        account_type: account.account_type || account.parent || 'N/A',
        status: account.pushed_to_zoho ? 'Completed' : 'Pending'
      }));

      console.log("Processed COA activities data:", transformedActivities);
      ...
      console.error('Error fetching COA dashboard data:', error);
      
      // Show error alert with detailed message
      showSnackbarAlert('error', `Failed to load COA dashboard data: ${error.message}`);
  ...
  useEffect(() => {
    console.log("COA Component mounted, starting data fetch...");
    ...
    const intervalId = setInterval(() => {
      console.log("Interval triggered, fetching COA data...");
      fetchAllDashboardData();
    }, 10000); // 10 seconds = 10000 milliseconds

    // Cleanup interval on component unmount
    return () => {
      console.log("COA Component unmounting, clearing interval...");
        ...
        <div className="main-content">
          ...
          <div className="header">
            <div className="header-left">
              ...
              <p>Monitor and manage your chart of accounts migration process from a single dashboard</p>
            </div>
          ...
          <div style={{ background: '#f0f0f0', padding: '10px', margin: '10px 0', fontSize: '12px' }}>
            ...
            COA Records Count: {recentActivities.length}<br/>
          ...
          <div className="stats-grid">
            <div className="stat-card blue">
              ...
              <div className="stat-content">
                <h3>Accounts Fetched from Tally</h3>
            ...
            <div className="stat-card orange">
              ...
              <div className="stat-content">
                <h3>Accounts Migrated to Zoho Books</h3>
                <div className="stat-number">
                    ...
                    {recentActivities.length > 0 ? (
                      recentActivities.map((account) => (
                        <tr key={account.sNo}>
                          ...
                          <td>
                            <span className={`status ${account.status.toLowerCase().replace(' ', '-')}`}>
                              {account.status}
                            </span>
                          </td>
                        </tr>
                        ...
                        <td colSpan="5" style={{ textAlign: 'center', padding: '40px' }}>
                          {isLoading ? (
                            ...
                            <div className="no-data">
                              ...
                              <p>No Chart of Accounts Data Found: Manage Migration from One Dashboard</p>
                            </div>
                          )}
                        </td>
                      </tr>
