...
function Masters() {
  ...
  const [counts, setCounts] = useState({ customers: 0, vendors: 0, accounts: 0, items: 0 });
  ...
  const fetchMasters = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to fetch masters: ${response.status} - ${err}`);
      }
  ...
  const totalFetched = counts.customers + counts.vendors + counts.accounts + counts.items;
      ...
      <div className="dashboard-container">
        ...
        <div className="main-content">
          ...
          <div className="stats-grid">
            ...
            <div className="stat-card yellow1">
              <div className="stat-content">
                ...
                <p>The sync process for master fields in Tally2Books keeps data updated across the system.</p>
              </div>
              <div className="sync-button-container">
                <button className="sync-button" onClick={fetchMasters}>Refresh</button>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '12px', margin: '12px 0', flexWrap: 'wrap' }}>
            {[
              { label: 'Customers', value: counts.customers, color: '#3B82F6' },
              { label: 'Vendors', value: counts.vendors, color: '#10B981' },
              { label: 'Accounts', value: counts.accounts, color: '#6B7280' },
              { label: 'Items', value: counts.items, color: '#F59E0B' },
            ].map(item => (
              ...
              <div className="table-container">
                ...
                {!loading && activities.length === 0 && (
                  <div className="no-data">
                    ...
                    <p>No data yet — run the Sync Agent to fetch masters from Tally</p>
                  </div>
                )}
                {loading && (
                  <div className="no-data">
